...
- มี "Sample Kit ชุดทดลองชง 4 ชนิด" ให้ร้านคาเฟ่นำไปเทสต์สูตรจริง
...
async function callGemini(message, history = []) {
  ...
  const payload = {
    system_instruction: { parts: [{ text: MATCHA_SYSTEM_PROMPT }] },
    contents,
    generationConfig: {
      temperature: 0.7,
    ...
    req.on('end', async () => {
      try {
        ...
        res.end(JSON.stringify({
          error: 'ขออภัยครับ ขณะนี้ระบบ AI กำลังประมวลผล กรุณาลองใหม่อีกครั้ง หรือทักสอบถามพี่เซลล์ผู้ดูแลทาง LINE ได้เลยครับ 🍵',
          details: err.message
        }));
      }
    });
